...
class dsp_adapter : public decorator_dsp {
    ...
    public:
        ...
        dsp_adapter(::base_dsp* base_dsp, int hw_inputs, int hw_outputs, int buffer_size, bool to_delete = true):decorator_dsp(base_dsp)
        {
            fHWInputs = hw_inputs;
            fHWOutputs = hw_outputs;
            fDSPInputs = base_dsp->getNumInputs();
            fDSPOutputs = base_dsp->getNumOutputs();
            fBufferSize = buffer_size;
            fDelete = to_delete;
            
            fAdaptedInputs = new FAUSTFLOAT*[std::max<int>(base_dsp->getNumInputs(), hw_inputs)];
            ...
            fAdaptedOutputs = new FAUSTFLOAT*[std::max<int>(base_dsp->getNumOutputs(), hw_outputs)];
...
struct MidiMeta : public Meta, public std::map<std::string, std::string> {
    ...
    static void analyse(base_dsp* mono_dsp, bool& midi, bool& midi_sync, int& nvoices)
    {
        JSONUI jsonui;
...
struct dsp_poly_factory : public dsp_factory {
    ...
    dsp_poly* createPolyDSPInstance(int nvoices, bool control, bool group, bool is_double = false)
    {
        if (nvoices == -1) {
            ...
            ::base_dsp* base_dsp = fProcessFactory->createDSPInstance();
            bool midi_sync = false;
            bool midi = false;
            MidiMeta::analyse(base_dsp, midi, midi_sync, nvoices);
            delete base_dsp;
        }
...
class SoundUI : public SoundUIInterface
{
    ...
    protected:
     ...
     public:
         ...
         *
         * @param base_dsp - the DSP.
         *
         * @return the list of paths.
         */
       
        static std::vector<std::string> getSoundfilePaths(base_dsp* base_dsp)
        {
...
struct mydsp : public base_dsp {
	...
	void metadata(Meta* m) { 
		...
		m->declare("filename", "snh_saw_synth_test.dsp");
		m->declare("maths.lib/author", "GRAME");
		m->declare("maths.lib/copyright", "GRAME");
		m->declare("maths.lib/license", "LGPL with exception");
		m->declare("maths.lib/name", "Faust Math Library");
		m->declare("maths.lib/version", "2.8.0");
		m->declare("name", "snh_saw_synth_test");
	...
	virtual void buildUserInterface(UI* ui_interface) {
		ui_interface->openVerticalBox("snh_saw_synth_test");
		ui_interface->addHorizontalSlider("alias", &fHslider1, FAUSTFLOAT(0.01f), FAUSTFLOAT(0.01f), FAUSTFLOAT(0.99f), FAUSTFLOAT(0.01f));
	...
	virtual void compute(int count, FAUSTFLOAT** RESTRICT inputs, FAUSTFLOAT** RESTRICT outputs) {
		...
		float fSlow3 = std::max<float>(1.1920929e-07f, std::fabs(fSlow0));
		float fSlow4 = fConst1 * fSlow3;
		float fSlow5 = 1.0f - fConst0 / fSlow3;
		float fSlow6 = float(fButton0) * float(fHslider2);
		for (int i0 = 0; i0 < count; i0 = i0 + 1) {
			iVec0[0] = 1;
			float fTemp0 = ((1 - iVec0[1]) ? 0.0f : fSlow1 + fRec1[1]);
			fRec1[0] = fTemp0 - std::floor(fTemp0);
			float fTemp1 = fSlow4 + fRec2[1] + -1.0f;
			int iTemp2 = fTemp1 < 0.0f;
			float fTemp3 = fSlow4 + fRec2[1];
			fRec2[0] = ((iTemp2) ? fTemp3 : fTemp1);
			float fRec3 = ((iTemp2) ? fTemp3 : fSlow4 + fRec2[1] + fSlow5 * fTemp1);
			fRec0[0] = ((int(fSlow2 + fRec1[0])) ? fSlow6 * (2.0f * fRec3 + -1.0f) : fRec0[1]);
	...
	#define FAUST_FILE_NAME "snh_saw_synth_test.dsp"
	...
	FAUST_ADDHORIZONTALSLIDER("alias", fHslider1, 0.01f, 0.01f, 0.99f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("freq", fHslider0, 2e+02f, 5e+01f, 1e+03f, 0.01f);
	FAUST_ADDHORIZONTALSLIDER("gain", fHslider2, 0.5f, 0.0f, 1.0f, 0.01f);
	FAUST_ADDBUTTON("gate", fButton0);

	#define FAUST_LIST_ACTIVES(p) \
		p(HORIZONTALSLIDER, alias, "alias", fHslider1, 0.01f, 0.01f, 0.99f, 0.01f) \
		p(HORIZONTALSLIDER, freq, "freq", fHslider0, 2e+02f, 5e+01f, 1e+03f, 0.01f) \
		p(HORIZONTALSLIDER, gain, "gain", fHslider2, 0.5f, 0.0f, 1.0f, 0.01f) \
